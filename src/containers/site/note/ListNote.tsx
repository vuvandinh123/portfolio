...
export default function ListNote() {
  const [data, setData] = React.useState<ListNoteType[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const { payload, status }: any = await noteApiRequest.getList({
        ...
        setData(payload.data as ListNoteType[]);
      } catch (error) {
  ...
  return (
    <div>
      ...
      {<div className="text-center text-gray-400">{error ? error : ""}</div>}
